...
def get_available_courses(completed_courses, prerequisites, total_credit):
    available_courses = set()
    total_credit = int(total_credit)  # Convertir total_credit a entero para comparación

    for course, reqs in prerequisites.items():
        # Ignorar cursos ya completados
        if course in completed_courses:
            continue

        # Filtrar prerequisitos para dividir los de curso y créditos
        credit_prereqs = [int(req[1:]) for req in reqs if req.startswith('C')]
        course_prereqs = [req for req in reqs if not req.startswith('C')]

        # Verificar que todos los cursos prerrequisitos estén completados
        if set(course_prereqs).issubset(completed_courses):
            # Verificar que los créditos sean suficientes
            if all(total_credit >= credit for credit in credit_prereqs):
                available_courses.add(course)

    return available_courses

# Función para obtener creditos totales
def get_total_credits(completed_courses, creditos):
    total_credits = 0
    for course, reqs in creditos.items():
